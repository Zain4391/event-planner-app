...
import { Calendar, MapPin, Users, Clock, Image as ImageIcon, Loader2 } from 'lucide-react';
import { Category, CategoryListResponse, Event } from '../../types/api';
import { apiClient, CreateEventData } from '../../lib/api';
...
export function CreateEventForm({ onSuccess, onCancel }: CreateEventFormProps) {
  ...
  const {
    ...
    defaultValues: {
      eventDate: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Tomorrow's date
      startTime: '18:00:00',
      endTime: '22:00:00',
      totalCapacity: 100,
    },
  ...
  const onSubmit = async (data: CreateEventFormData) => {
    try {
      ...
      // Clean up the data to match CreateEventData type
      const eventData: CreateEventData = {
        ...
        eventDate: data.eventDate, // Send as YYYY-MM-DD format as expected by backend
      ...
      console.log('eventDate type:', typeof eventData.eventDate);
      console.log('eventDate value:', eventData.eventDate);

      const response = await apiClient.events.create(eventData);
