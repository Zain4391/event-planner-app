...
export const users = pgTable(
  ...
  (table) => [
    ...
    check(
      'users_role_check',
      sql`(role)::text = ANY (ARRAY[('Admin'::character varying)::text, ('Organizer'::character varying)::text, ('Customer'::character varying)::text])`,
    ),
  ],
);
...
export const payments = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(status)::text = ANY (ARRAY[('Pending'::character varying)::text, ('Completed'::character varying)::text, ('Failed'::character varying)::text, ('Refunded'::character varying)::text])`,
    ),
  ],
);
...
export const ticketTiers = pgTable(
  ...
  (table) => [
    index('idx_ticket_tiers_event').using(
      'btree',
...
export const events = pgTable(
  ...
  (table) => [
    ...
    index('idx_events_date').using(
      'btree',
      table.eventDate.asc().nullsLast().op('timestamp_ops'),
    ),
    index('idx_events_organizer').using(
      'btree',
    ...
    check(
      'events_status_check',
      sql`(status)::text = ANY (ARRAY[('Draft'::character varying)::text, ('Published'::character varying)::text, ('Cancelled'::character varying)::text, ('Completed'::character varying)::text])`,
    ),
  ],
);
...
export const orders = pgTable(
  ...
  (table) => [
    ...
    check(
      'orders_status_check',
      sql`(status)::text = ANY (ARRAY[('Pending'::character varying)::text, ('Confirmed'::character varying)::text, ('Cancelled'::character varying)::text, ('Expired'::character varying)::text])`,
    ),
  ],
);
...
export const tickets = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`(status)::text = ANY (ARRAY[('Active'::character varying)::text, ('Used'::character varying)::text, ('Cancelled'::character varying)::text, ('Refunded'::character varying)::text])`,
    ),
  ],
);
